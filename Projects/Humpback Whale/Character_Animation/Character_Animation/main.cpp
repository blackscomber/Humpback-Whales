...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed)
{
	...
	if(FAILED(d3d9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hMainWindow, vp, &m_present, &g_pDevice)))
	{
	...
	//Load Soldier Mesh
	m_soldier.LoadMeshFromX("resources/meshes/soldier.x");
	m_bDeviceLost = false;
...
void Application::Render()
{
	if(!m_bDeviceLost)
	{
		try
		{
			...
			D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(cos(m_fAngle) * 2.0f, 2.0f, sin(m_fAngle) * 2.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
